...
def list_sections(response: Response, user: dict = Depends(get_current_user)):
...
def list_items(section: str, response: Response, user: dict = Depends(get_current_user)):
    response.headers["Cache-Control"] = "private, max-age=60, stale-while-revalidate=120"
    return get_items_by_section(section)
...
def patch_item(item_id: str, req: UpdateCodebookItemRequest, user: dict = Depends(require_admin)):
